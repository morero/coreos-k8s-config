...
k8s-keys/ca.pem: k8s-keys/ca-key.pem
	openssl req -x509 -new -nodes -key "$<" -days 10000 -out "$@" \
  -subj "/CN=kube-ca"

k8s-keys/apiserver.csr: k8s-keys/apiserver-key.pem openssl.cnf
	openssl req -new -key "$<" -out "$@" -subj "/CN=kube-apiserver" \
  -config openssl.cnf

k8s-keys/coreos0-worker.csr: k8s-keys/coreos0-worker-key.pem worker-openssl.cnf
	WORKER_FQDN=coreos0 WORKER_IP=$(COREOS0) openssl req -new -key "$<" \
  -out "$@" -subj "/CN=coreos0" -config worker-openssl.cnf

k8s-keys/coreos0-worker.pem: k8s-keys/coreos0-worker.csr k8s-keys/ca.pem \
                             k8s-keys/ca-key.pem worker-openssl.cnf
	WORKER_FQDN=coreos0 WORKER_IP=$(COREOS0) openssl x509 -req -in "$<" \
  -CA k8s-keys/ca.pem -CAkey k8s-keys/ca-key.pem -CAcreateserial -out "$@" \
...
k8s-keys/coreos1-worker.csr: k8s-keys/coreos1-worker-key.pem worker-openssl.cnf
	WORKER_FQDN=coreos1 WORKER_IP=$(COREOS1) openssl req -new -key "$<" \
  -out "$@" -subj "/CN=coreos1" -config worker-openssl.cnf

k8s-keys/coreos1-worker.pem: k8s-keys/coreos1-worker.csr k8s-keys/ca.pem \
                             k8s-keys/ca-key.pem worker-openssl.cnf
	WORKER_FQDN=coreos1 WORKER_IP=$(COREOS1) openssl x509 -req -in "$<" \
  -CA k8s-keys/ca.pem -CAkey k8s-keys/ca-key.pem -CAcreateserial -out "$@" \
  -days 365 -extensions v3_req -extfile worker-openssl.cnf

k8s-keys/coreos2-worker.csr: k8s-keys/coreos2-worker-key.pem worker-openssl.cnf
	WORKER_FQDN=coreos2 WORKER_IP=$(COREOS2) openssl req -new -key "$<" \
  -out "$@" -subj "/CN=coreos2" -config worker-openssl.cnf

k8s-keys/coreos2-worker.pem: k8s-keys/coreos2-worker.csr k8s-keys/ca.pem \
                             k8s-keys/ca-key.pem worker-openssl.cnf
	WORKER_FQDN=coreos2 WORKER_IP=$(COREOS2) openssl x509 -req -in "$<" \
  -CA k8s-keys/ca.pem -CAkey k8s-keys/ca-key.pem -CAcreateserial -out "$@" \
  -days 365 -extensions v3_req -extfile worker-openssl.cnf

k8s-keys/apiserver.pem: k8s-keys/apiserver.csr k8s-keys/ca.pem \
                        k8s-keys/ca-key.pem openssl.cnf
	openssl x509 -req -in "$<" -CA k8s-keys/ca.pem -CAkey k8s-keys/ca-key.pem \
  -CAcreateserial -out "$@" -days 365 -extensions v3_req -extfile openssl.cnf

k8s-keys/admin.csr: k8s-keys/admin-key.pem
	openssl req -new -key "$<" -out "$@" -subj "/CN=kube-admin"
...
deploy-coreos0: cloud-config/coreos0-cloud-config.yaml keys
	scp "$<" k8s-keys/ca.pem k8s-keys/coreos0-*.pem coreos0:/home/core
	...
	ssh coreos0 sudo mv $(notdir $<) /var/lib/coreos-install/user_data
...
deploy-coreos1: cloud-config/coreos1-cloud-config.yaml keys
	scp "$<" k8s-keys/ca.pem k8s-keys/coreos1-*.pem coreos1:/home/core
	...
	ssh coreos1 sudo mv $(notdir $<) /var/lib/coreos-install/user_data
...
deploy-coreos2: cloud-config/coreos2-cloud-config.yaml keys
	scp "$<" k8s-keys/ca.pem k8s-keys/coreos2-*.pem coreos2:/home/core
	...
	ssh coreos2 sudo mv $(notdir $<) /var/lib/coreos-install/user_data
...
deploy-coreos3: cloud-config/coreos3-cloud-config.yaml keys
	scp "$<" k8s-keys/ca.pem k8s-keys/apiserver*.pem coreos3:/home/core
	...
	ssh coreos3 sudo mv $(notdir $<) /var/lib/coreos-install/user_data
